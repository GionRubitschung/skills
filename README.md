...
- **Output:** `workflow.json` with the ticket DAG, the baseline (`feat/<slug>` in `.claude/worktrees/<slug>`), the verification commands with their timeout, the setup commands every worktree runs once, the models per role and `maxTurns`.
