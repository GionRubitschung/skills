...
- Verification commands: from CLAUDE.md, `package.json` scripts, Makefile and CI config, pick the project's test, lint, typecheck and build commands. Each runs non-interactively from the worktree root, exits non-zero on failure, and is independent of the others: the verifier runs them all at once, so a dependent pair is one command joined with `&&`. Run all of them at once on the current tree as background Bash calls and wait for every completion; `verifyTimeoutMs` is five times the longest wall clock, at least 600000.
- Setup commands: from the same sources, the project's dev setup — a Makefile target named setup, install, dev or bootstrap, or the install command (`npm ci`, `pnpm install`, `uv sync`, `poetry install`, …), plus a copy of any gitignored file agents need (`cp <repo>/.env .`). In order, each run from a worktree's root right after the worktree is created; none when the project has no install step.
...
**Done when:** every remaining ticket is classified, the verification commands are chosen (fewer than four only when the project lacks that step) and timed, the setup commands are chosen or known to be none, all four guards checked.
