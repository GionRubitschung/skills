...
- **Verification commands**: from CLAUDE.md, `package.json` scripts, Makefile and CI config, pick the project's test, lint, typecheck and build commands, each non-interactive, run from `dir`, non-zero on failure, and independent of the others: the verifier runs them all at once, so a dependent pair is one command joined with `&&`. Run them all at once as background Bash calls and wait for every completion; `verifyTimeoutMs` is five times the longest wall clock, at least 600000. Note which are red now.
- **Setup commands**: from the same sources, the project's dev setup — a Makefile target named setup, install, dev or bootstrap, or the install command (`npm ci`, `pnpm install`, `uv sync`, `poetry install`, …), plus a copy of any gitignored file agents need (`cp <repo>/.env .`). Run in order from the worktree's root once it is created; none when the project has no install step, and never in current-directory mode.
...
Show the user, in one message: the ticket text, `dir` and `branch`, the verification commands (with the ones already red) and `verifyTimeoutMs`, the setup commands or none, the models, `maxTurns`. Apply every change they ask for; nothing is created before they approve.

Then: create the worktree when chosen (`git worktree add .claude/worktrees/<slug> -b feat/<slug>`, without `-b` when the branch exists) and, when it was just created, run the setup commands in order from its root as foreground Bash calls with the `timeout` parameter set to `verifyTimeoutMs`; a non-zero exit stops the skill: show the last 15 lines of its output and say to fix it, run the setup in the worktree by hand, then run `/ship <ref>` again. Add the lines `.claude/worktrees/` and `.scratch/` to `.git/info/exclude` when missing. When `${BASH_MAX_TIMEOUT_MS:-600000}` is below `verifyTimeoutMs`: set `env.BASH_MAX_TIMEOUT_MS` to `verifyTimeoutMs` in `.claude/settings.json` (create it, or merge keeping every other key), and stop with `Restart Claude Code to pick up .claude/settings.json, then run /ship <ref> again.` A ticket written from the conversation is already on disk, so `<ref>` is its path.
